...
void parameterScanVSD(Junction junction, std::string filename)
{
    ...
    std::vector<double> eJs = {1e-6, 9e-7, 8e-7, 7e-7, 6e-7, 5e-7, 4e-7, 3e-7, 2e-7, 1e-7};
        ...
        for (const double &J : Js)
        {
            for (const double &eJ : eJs)
            {
                for (double field = minField; field < maxField; field += spacing)
                {
                    ...
                    std::map<std::string, double> vsd = junction.calculateVoltageSpinDiode(F);
                    vsdFile << F << ";" << J << ";" << eJ << ";" << field << ";" << vsd["Vmix"] << ";" << vsd["Rpp"] << std::endl;
                    junction.log.clear();
                }
            }
        }
...
{
    // std::vector<double> Js = {-4e-6, -3e-4, -2e-4, -1e-4, 0.0, 1e-4, 2e-4, 3e-4, 4e-4};
    // std::vector<double> Js = {-9e-6, -8e-6, -7e-6, -6e-6, -5e-6, -4e-6, -3e-6, -2e-6, -1e-6, 0.0,
    //                           1e-6, 2e-6, 3e-6, 4e-6, 5e-6, 6e-6, 7e-6, 8e-6, 9e-6};
    std::vector<double> Js = {-9e-5, -8e-5, -7e-5, -6e-5, -5e-5, -4e-5, -3e-5, -2e-5, -1e-5, 0.0,
                              1e-5, 2e-5, 3e-5, 4e-5, 5e-5, 6e-5, 7e-5, 8e-5, 9e-5};
    std::vector<double> Ks = {900e3, 950e3, 1000e3, 1050e3, 1100e3, 1150e3};
    std::vector<double> fixedFields = {150_mT, 200_mT, 250_mT,
                                       300_mT, 350_mT, 400_mT, 450_mT};
    ...
    for (const double &J : Js)
    {
        for (const double &K : Ks)
        {
            for (const double &H : fixedFields)
            {
                ...
                junction.setLayerStepUpdate("free", 1e-4 * TtoAm, 5.0_ns, 5.001_ns, xaxis);
                junction.setLayerStepUpdate("bottom", 1e-4 * TtoAm, 5.0_ns, 5.001_ns, xaxis);
                ...
                {
                    for (const std::string minorKey : {"_resonant", "_amplitude", "_phase"})
                    {
...
int main(void)
{
    ...
    //                                          [](Junction &mtj, const double field) mutable {
    //                                              mtj.log.clear();
    //                                              const double freq = 7e9;
    //                                              mtj.setConstantExternalField((field / 1000) * TtoAm, xaxis);
    //                                              mtj.setLayerCoupling("free", -3e-6);
    //                                              mtj.setLayerCoupling("bottom", -3e-6);
    //                                              mtj.setLayerIECUpdate("free", 1e-6, freq, 0);
    //                                              mtj.setLayerIECUpdate("bottom", 1e-6, freq, 0);
    //                                              mtj.runSimulation(20e-9);
    //                                              std::map<std::string, double> vsd = mtj.calculateVoltageSpinDiode(freq);
    //                                              mtj.log.clear();
    //                                              return std::make_tuple(field, vsd);
    //                                          });
