...
cdef calculate_tensor_interaction(m, tensor, double Ms):
...
class Layer:
    ...
    def calculate_interlayer_exchange_coupling_field(self, coupled_layers_m=None):
        ...
        heff_iec = [(coupled_layers_m.m[i] - self.m[i]) for i in range(3)] 
        return [self.coupling*heff_iec_v/(MAGNETIC_PERMEABILITY*self.Ms*self.thickness)
                    for heff_iec_v in heff_iec]
...
class Junction():
    ...
    def run_simulation(self, stop_time, time_step=1e-13):
        ...
        for i in range(0, iterations):
            ...
            self.layers[0].rk4_step(t, time_step, self.layers[1])
            self.layers[1].rk4_step(t, time_step, self.layers[0])
