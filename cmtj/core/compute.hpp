...
class ComputeFunctions
{
                ...
                double minTime = 10.0e-9, double timeStep = 1e-11)
    {
        ...
        for (const auto &id : fftIds)
        {
            for (const auto &magTag : vectorNames)
            {
                std::vector<double> cutMag(log[id + "_m" + magTag].begin() + thresIdx, log[id + "_m" + magTag].end());
                ...
                spectralFFTResult[id + "_m" + magTag + "_amplitude"] = std::move(amplitudes);
