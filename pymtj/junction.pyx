...
class Layer:
    ...
    def Heff(self, m, time, coupled_layers):
        ...
        iec = self.calculate_interlayer_exchange_coupling_field(
            m, coupled_layers)
        ti_demag = calculate_tensor_interaction(m, self.demagnetisation_tensor,
                                                self.Ms)
    ...
    def calculate_anisotropy(self, m):
        ...
        Units:
            returns the value in A/m
        """
        nom = 2 * self.K_log * c_dot(
            m, self.anisotropy) / (MAGNETIC_PERMEABILITY * self.Ms)
...
class Junction():
    ...
    def set_junction_global_external_field(self,
                                           ...
                                           axis='x'):
        """
        Set the exteranl magnetic field for all layers in the junction
    ...
    def set_global_field_function(self, field_function):
        ...
        :param field_function
            function that updates the exteral field at each
            time step
        Units:
            returns the value in A/m
        """
    ...
    def set_global_anisotropy_function(self, anisotropy_function):
        ...
        :param field_function
            function that updates the exteran field at each
            time step
        Units:
            returns the value in A/m
        """
